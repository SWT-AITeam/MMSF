...
class Sentence_RE(nn.Module):
    ...
    def train_semeval_model(self, warmup=True):
        ...
        for epoch in range(self.max_epoch):
            ...
            # print("train_singla_loader",train_singla_loader)
            ...
            # print("binary_f1: %.4f" % (result['binary_f1']))
            print("sk-acc: %.4f" % (result['sk-acc']))
            print("recall: %.4f" % (result['recall']))
            print("precision: %.4f" % (result['precision']))
            # out = {'macro_f1': macro_f1, 'micro_f1': micro_f1, 'binary_f1':binary_f1, 'acc': avg_acc.avg, 'sk-acc':acc,'recall':recall,'precision':precision }
    ...
    def train_once(self, loader, warmup=True):
        ...
        for iter, data in enumerate(t):
            ...
            sent_label = data[0]
            args = data[1:]
            logits, _ , kld= self.model(sent_label, *args)
            # print()
            l = torch.matmul(kld, sent_label.T.type(torch.float32)).sum()
            # print('l',l)
            # loss
            ...
            loss, acc = self.acc_loss(logits, sent_label,l, 4 )
            # print("loss1",loss1)
            # print("l*self.model.config.beta",l*self.model.config.beta)
    ...
    def eval_semeval(self, eval_loader):
        ...
        with torch.no_grad():
            ...
            for iter, data in enumerate(t):
                ...
                logits, pred, kld = self.model(label, *args)
            ...
            macro_f1 = f1_score(label_tot, pred_result, average='macro')
            micro_f1 = f1_score(label_tot, pred_result, average='micro')
            # binary_f1 = sklearn.metrics.f1_score(label_tot, pred_result, average='binary')
            recall = recall_score(label_tot, pred_result, average='macro')
            precision = precision_score(label_tot, pred_result, average='macro')
            # auc = sklearn.metrics.auc(x, y, reorder=False)
            # auc = sklearn.metrics.roc_auc_score(y_true, y_score, average=‘macro’, sample_weight=None)
            acc = accuracy_score(label_tot, pred_result, normalize=True, sample_weight=None)
            out = {'macro_f1': macro_f1, 'micro_f1': micro_f1, 'acc': avg_acc.avg, 'sk-acc':acc,'recall':recall,'precision':precision }
            # office script
    ...
    def acc_loss(self, logits, sent_label, kld, nums=4):
        y = Variable(torch.eye(nums)) #torch.eye(nums)对角线全1，其他全零
        if torch.cuda.is_available():
            y = y.cuda()
        y = y.index_select(dim=0, index=sent_label.data)
        _, pred = torch.max(logits.view(-1, nums), 1)
        # print('logits.view(-1, nums)',logits.view(-1, nums))
        acc = float((pred == sent_label).long().sum()) / sent_label.size(0)
        sent_loss = self.loss_func(logits, y) + kld*self.model.config.beta
